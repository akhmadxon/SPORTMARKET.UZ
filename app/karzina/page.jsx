...
import Img from "@/public/pngegg (59).png";
  ...
  <div className="w-full max-w-[504px] h-[800px] mx-auto bg-white shadow-lg rounded-lg overflow-hidden">
    <div className="p-6">
      ...
      <div className="flex justify-between mb-6">
        <span>Сумма:</span>
        <span className="font-semibold">{totalPrice.toLocaleString()} UZS</span>
      </div>
...
const ProductItem = ({ product, incrementCount, decrementCount, deleteCard }) => (
  <div className="font-Fira-sans max-w-[655px] h-[140px] flex border rounded-lg overflow-hidden p-4 bg-custom-gray relative mb-4">
    <button
    ...
    <div className="flex flex-col sm:flex-col lg:flex-row">
      <div className="w-[145px] h-[120px] flex-shrink-0 mr-4">
        <Image src={product.image} alt={product.name} width={145} height={120} />
      </div>
      <div className="flex-1 flex flex-col justify-between">
        ...
        <div className="flex items-center gap-[6px]">
          ...
          <span className="pl-[20px] text-[22px] font-bold">
            {(product.count * product.pricePerUnit).toLocaleString()} uzs
          </span>
        </div>
      </div>
    </div>
...
const ProductList = ({ products, incrementCount, decrementCount, deleteCard }) => (
  <div className="w-full max-w-[713px] h-auto rounded-lg bg-white p-4">
    <div className="flex justify-between items-center pt-[20px] pb-[15px]">
      ...
      <button onClick={() => deleteCard()} className="text-red-500">
        Очистить все
      </button>
    </div>
    ...
    <p className="max-w-[384px]">
      Если у вас имеется вопросы позвоните по номеру: +998 (99) 995 55 65
    </p>
  </div>
);

const ProductCard = () => {
  const [products, setProducts] = useState(productData.map((product) => ({ ...product, count: 0 })));
  ...
  const totalQuantity = products.reduce((total, product) => total + product.count, 0);
  const totalPrice = products.reduce((total, product) => total + product.count * product.pricePerUnit, 0);
