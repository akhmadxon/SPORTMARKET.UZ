...
import { getComment, addComment } from "@/components/service/comment";
import toast from "react-hot-toast";
import User from "@/public/usert.png";
    ...
    >
      {images.map((image, index) => (
        <SwiperSlide key={index}   style={{
          ...
          alignItems: "center",}}>
          <Image src={image} alt={`Product image ${index + 1}`} width={370} height={370} priority />
        </SwiperSlide>
      ))}
    </Swiper>
    <div className="flex gap-2 mt-4">
      {images.map((image, index) => (
        <div key={index} className="w-20 h-20 bg-white p-2 border-2 border-yellow-500 rounded-lg overflow-hidden">
          <Image src={image} alt={`Thumbnail image ${index + 1}`} className="w-full h-full object-cover" width={370} height={370} priority />
        </div>
      ))}
    </div>
  </>
...
const Index = () => {
  ...
  const handleBasket = async (id) => {
    try {
      ...
      response.data ? toast.success("Product added to basket successfully") : toast.error("Failed to add product to basket");
    } catch (error) {
  ...
  const handleAddComment = async (e) => {
    ...
    try {
      ...
      await addComment(product_id, commentText);
    } catch (error) {
      console.log(error);
    }
  };
        ...
        <div className="flex flex-col w-full md:flex-row gap-8 p-6 rounded-lg bg-white">
          ...
          <div className="md:w-1/2 md:p-8 p-2 bg-white">
            ...
            <div className="flex gap-4 mt-[30px]">
              <button onClick={() => handleBasket(product_id)} className="flex items-center gap-[4px] px-5 py-3 bg-yellow-400 text-[#1F1D14] rounded-md">
        ...
        <div className="mt-[40px]">
          <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-8 container">
            <div className="bg-white p-6 rounded-lg shadow-md transition-transform hover:scale-105">
              ...
              <h4 className="text-[24px] font-medium text-[#1F1D14] mb-4">{product_name}</h4>
              <p className="text-[16px] text-[#000] font-normal mb-5">{description}</p>
            ...
            <div className="bg-white p-6 rounded-lg shadow-md transition-transform hover:scale-105">
                ...
                visibleComments.map((item, index) => (
                  <div key={index} className="mb-4">
                    <div className="flex gap-4 items-center">
                      <Image src={User} alt="User avatar" width={50} height={50} />
                      <div>
                    ...
                    <p className="text-[16px] text-[#000] font-normal mt-2">{item.Message}</p>
              ...
              <p className="mt-4 text-[16px] text-[#1F1D14] font-normal opacity-60 flex justify-between cursor-pointer" onClick={() => setShowAllComments(!showAllComments)}>
              ...
              <form id="submit" onSubmit={handleAddComment}>
                <textarea
                  ...
                  rows={4}
                />
                <button type="submit" className="flex items-center gap-[4px] px-5 py-3 bg-yellow-400 text-[#1F1D14] rounded-md">
                  Отправить
                </button>
              </form>
            </div>
